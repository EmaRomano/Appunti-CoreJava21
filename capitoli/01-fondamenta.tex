...
\begin{tip}[Il metodo \texttt{intern()}]
    Esiste un modo per forzare una stringa dinamica nel Pool: il metodo \texttt{s.intern()}. Se invocato, Java controlla se esiste già una stringa uguale nel Pool: se sì, restituisce il riferimento esistente; altrimenti, aggiunge la stringa al Pool e ne restituisce il riferimento.
...
L'uso della stringa \texttt{"0.1"} garantisce invece la precisione esatta richiesta.
\end{warningbox}
