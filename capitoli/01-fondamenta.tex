...
\begin{enumerate}
    ...
    \textbf{\textcolor{green!60!black}{[DA RIVEDERE DOPO AVER FATTO BENE HASHCODE E COLLECTIONS]}}Poiché il contenuto di una \texttt{String} non cambia mai, Java ne calcola l'hashcode una sola volta e lo memorizza in \textit{cache}. Questo rende le stringhe estremamente veloci quando usate come chiavi nelle \texttt{HashMap} o nei \texttt{HashSet}. Se la stringa fosse mutabile, l'hash andrebbe ricalcolato ad ogni operazione di ricerca o inserimento.
...
\begin{tip}[Il metodo \texttt{intern()}]
    Esiste un modo per forzare una stringa dinamica nel Pool: il metodo \texttt{s.intern()}. Se invocato, Java controlla se esiste gia una stringa uguale nel Pool: se sì, restituisce il riferimento esistente; altrimenti, aggiunge la stringa allo Pool e ne restituisce il riferimento.
