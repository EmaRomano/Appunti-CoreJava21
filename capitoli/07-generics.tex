...
\subsection{I Metodi Ponte (Bridge Methods)}

I \textit{bridge methods} sono sintetizzati dal compilatore per gestire le discrepanze causate dalla \textit{type erasure}.

Data una gerarchia \texttt{Sub extends Super<T>}, il processo segue questi passi:
\begin{enumerate}
    \item La \textit{type erasure} trasforma i metodi della superclasse in metodi \textit{raw} (usando \texttt{Object} o i \textit{bounds}).
    \item Il compilatore crea un metodo nella sottoclasse con la firma \textit{raw} (il "ponte").
    \item Il ponte effettua il cast del parametro al tipo specifico e invoca il metodo originale della sottoclasse.
\end{enumerate}

Sebbene il linguaggio Java proibisca metodi con firme identiche ma diversi tipi di ritorno, la JVM permette questa distinzione nel \textit{bytecode}. I Generics sfruttano questa caratteristica per mantenere il polimorfismo anche quando i tipi vengono cancellati.
